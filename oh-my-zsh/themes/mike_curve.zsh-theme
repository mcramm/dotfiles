...
get_prompt() {
    ...
    if [ "${CURVEPROJECT}" = "" ]; then
        echo "${PR_CYAN}%n@%m${PR_RESET}$(git_prompt_info)${PR_RESET}${PR_YELLOW} \$ ${PR_RESET}"
    else
        ...
        ${CURVESPACE}/${CURVEPROJECT}*)
            echo "${PR_GREEN}$CURVEPROJECT${PR_RESET}${PR_YELLOW}$(git_prompt_info) ${PR_YELLOW}\$ ${PR_RESET}"
        ;;
        *)
...
get_right_prompt() {
    echo "${PR_YELLOW}${PWD/#$HOME/~}/${PR_RESET}"
}
...
ZSH_THEME_GIT_PROMPT_PREFIX=" ${PR_CYAN}(${PR_RESET}${PR_YELLOW}" 
